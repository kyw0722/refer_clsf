...
def _define_model(input_shape, networks, dropout_rate, learning_rate, class_cnt):
    ...
    for idx, node_cnt in enumerate(networks):
        if idx == 0:
            model.add(keras.layers.Dense(node_cnt, input_shape=(input_shape,), kernel_initializer='he_normal'))

            model.add(keras.layers.BatchNormalization())
            model.add(keras.layers.ELU())
            model.add(keras.layers.Dropout(dropout_rate))
        else:
            # model.add(keras.layers.Dense(node_cnt, activation=hidden_activation, kernel_initializer='he_normal'))
            model.add(keras.layers.Dense(node_cnt, kernel_initializer='he_normal'))
